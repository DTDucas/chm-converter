

Collapse AllExpand All Code: All Code: Multiple Code: C# Code: Visual Basic Code: Visual C++   
---  
  
C#Visual BasicVisual C++

Revit 2024 API  
---  
FillGrid..::..GetPointLineZone Method (UV, UV%)  
[FillGrid Class](6dfc3e2f-c869-d06e-876e-49c4007f7e59.md) See Also [Send Feedback](javascript:SubmitFeedback\('revitapifeedback@autodesk.com','Revit 2024 API','','','','%0\\dYour%20feedback%20is%20used%20to%20improve%20the%20documentation%20and%20the%20product.%20Your%20e-mail%20address%20will%20not%20be%20used%20for%20any%20other%20purpose%20and%20is%20disposed%20of%20after%20the%20issue%20you%20report%20is%20resolved.%20%20While%20working%20to%20resolve%20the%20issue%20that%20you%20report,%20you%20may%20be%20contacted%20via%20e-mail%20to%20get%20further%20details%20or%20clarification%20on%20the%20feedback%20you%20sent.%20After%20the%20issue%20you%20report%20has%20been%20addressed,%20you%20may%20receive%20an%20e-mail%20to%20let%20you%20know%20that%20your%20feedback%20has%20been%20addressed.%0\\A%0\\d','Customer%20Feedback'\);)  
---  
  
Gets the index of fill grid line and the point on the grid line nearest to the input point. 

**Namespace:** [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 24.0.0.0 (24.0.0.0)**Since:** 2012 

# Syntax

C#  
---  
      
    
    public int GetPointLineZone(
    	[UV](1724be37-059b-91ff-aa74-d1508082f76d.md) point,
    	out [UV](1724be37-059b-91ff-aa74-d1508082f76d.md) nearestPoint
    )  
  
Visual Basic  
---  
      
    
    Public Function GetPointLineZone ( _
    	point As [UV](1724be37-059b-91ff-aa74-d1508082f76d.md), _
    	<OutAttribute> ByRef nearestPoint As [UV](1724be37-059b-91ff-aa74-d1508082f76d.md) _
    ) As Integer  
  
Visual C++  
---  
      
    
    public:
    int GetPointLineZone(
    	[UV](1724be37-059b-91ff-aa74-d1508082f76d.md)^ point, 
    	[OutAttribute] [UV](1724be37-059b-91ff-aa74-d1508082f76d.md)^% nearestPoint
    )  
  
#### Parameters

point
    Type: [Autodesk.Revit.DB..::..UV](1724be37-059b-91ff-aa74-d1508082f76d.md) Input point. 

nearestPoint
    Type: [Autodesk.Revit.DB..::..UV](1724be37-059b-91ff-aa74-d1508082f76d.md)% The point on the grid line nearest to the input point. 

#### Return Value

The index of fill grid line. 

# Exceptions

| Exception | Condition |
| --- | --- |
| [Autodesk.Revit.Exceptions..::..ArgumentNullException](631e1424-60f4-929b-4e52-dda9dcd26316.md) | A non-optional argument was null |
  
# See Also

[FillGrid Class](6dfc3e2f-c869-d06e-876e-49c4007f7e59.md)

[GetPointLineZone Overload](3bbe339f-c7f0-a266-c698-2bb03e0f2e3e.md)

[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)

Send comments on this topic to [Autodesk](mailto:revitapifeedback%40autodesk.com?Subject=Revit 2024 API)