# MassGBXMLExportOptions Constructor (IList(ElementId), IList(ElementId)) (2023)
...
Revit 2023 API  
---  
MassGBXMLExportOptions Constructor (IList<(Of <(<'ElementId>)>)>, IList<(Of <(<'ElementId>)>)>)  
...
Public Sub New ( _
	...
	massIds As IList(Of ElementId) _
)
```
...
massIds
    Type: System.Collections.Generic..::..IList<(Of <(<'[ElementId](44f3f7b1-3229-3404-93c9-dc5e70337dd6.md "ElementId Class")>)>)> Indicates the masses to use as shading surfaces in the exported gbXML--these masses must not have mass floors or mass zones so as not to end up with duplicate surface information in the gbXML output. 
# Exceptions
| Exception | Condition |
| --- | --- |
| --- | --- |
